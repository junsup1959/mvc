...
//		Iterator<Theater_infoDTO>it = screenCodes.iterator();
		
//		while(it.hasNext()) {
//			System.out.println(it.next().getStart_time());
//			System.out.println(it.next().getEnd_time());
//			System.out.println(it.next().getStart_date());
//			System.out.println(it.next().getEnd_date());
		for(int i = 0;i<screenCodes.size();i++) {
			...
			if((i2_s>=s2_s&&i2_s<=s2_e) | (i2_e>=s2_s&&i2_e<=s2_e)) {
				//날짜가 겹칠때
				if((i_s>=s_s&&i_s<=s_e) | (i_e>=s_s&&i_e<=s_e))//시간이 겹칠때
					return "중복";
			}
		}
//		}
